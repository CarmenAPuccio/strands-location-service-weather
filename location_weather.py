...
stdio_mcp_client = MCPClient(lambda: stdio_client(
    StdioServerParameters(command="uvx", args=["awslabs.aws-location-mcp-server@latest"])
))
...
def get_weather(latitude: float, longitude: float) -> Dict[str, Any]:
    ...
    with tracer.start_as_current_span("get_weather_api") as span:
        ...
        headers = {
            "User-Agent": "WeatherToolExample/1.0",
        ...
        try:
            ...
            with tracer.start_as_current_span("get_grid_points") as grid_span:
                ...
                if points_response.status_code != 200:
                    error_msg = f"Failed to get grid data: {points_response.status_code}"
                    logger.error(error_msg)
                    span.set_attribute("error", error_msg)
                    return {"error": "Failed to get grid data", "status": points_response.status_code}
            ...
            with tracer.start_as_current_span("get_forecast") as forecast_span:
                ...
                forecast_span.set_attribute("status_code", forecast_response.status_code)
                
                if forecast_response.status_code != 200:
                    error_msg = f"Failed to get forecast data: {forecast_response.status_code}"
                    logger.error(error_msg)
                    span.set_attribute("error", error_msg)
                    return {"error": "Failed to get forecast data", "status": forecast_response.status_code}
            ...
            result = {
                ...
                "windSpeed": {
                    "value": current_period["windSpeed"],
                    "unit": "mph"
                },
            ...
            logger.info(f"Weather retrieved: {current_period['temperature']}°{current_period['temperatureUnit']}, {current_period['shortForecast']}")
            return result
...
def get_alerts(latitude: float, longitude: float) -> List[Dict[str, Any]]:
    ...
    with tracer.start_as_current_span("get_weather_alerts") as span:
        ...
        # Set headers for the request
        headers = {
            "User-Agent": "WeatherAlertsExample/1.0",
            "Accept": "application/geo+json"
        }
        ...
        try:
            ...
            with tracer.start_as_current_span("get_zone_info") as zone_span:
                ...
                if points_response.status_code != 200:
                    error_msg = f"Failed to get zone data: {points_response.status_code}"
                    logger.error(error_msg)
                    span.set_attribute("error", error_msg)
                    return [{"error": "Failed to get zone data", "status": points_response.status_code}]
            ...
            with tracer.start_as_current_span("get_alerts_data") as alerts_span:
                alerts_url = f"https://api.weather.gov/alerts/active/zone/{county_zone}"
                ...
                if alerts_response.status_code != 200:
                    error_msg = f"Failed to get alerts data: {alerts_response.status_code}"
                    logger.error(error_msg)
                    span.set_attribute("error", error_msg)
                    return [{"error": "Failed to get alerts data", "status": alerts_response.status_code}]
    ...
    def __init__(
        ...
        with tracer.start_as_current_span("agent_initialization") as span:
            try:
                ...
                prompt_to_use = custom_system_prompt if custom_system_prompt else system_prompt
        ...
        with tracer.start_as_current_span("agent_interaction") as span:
            try:
                ...
                with tracer.start_as_current_span("bedrock_model_inference") as model_span:
                    # Set a fixed model ID attribute
                    model_span.set_attribute("model_id", "anthropic.claude-3-sonnet-20240229-v1:0")
                    ...
                    try:
                        ...
                        model_span.set_status(trace.Status(trace.StatusCode.ERROR, str(model_error)))
                    ...
                    if hasattr(result, "metrics"):
                        try:
                            ...
                            if hasattr(result.metrics, "accumulated_usage") and isinstance(result.metrics.accumulated_usage, dict):
                            ...
                            if hasattr(result.metrics, "cycle_durations") and result.metrics.cycle_durations:
                                ...
                                model_span.set_attribute("metrics.total_duration", total_duration)
                                model_span.set_attribute("metrics.cycle_count", len(result.metrics.cycle_durations))
                            ...
                            if hasattr(result.metrics, "tool_metrics") and result.metrics.tool_metrics:
                                ...
                                model_span.set_attribute("metrics.tools_used", ", ".join(tools_used))
                                model_span.set_attribute("metrics.tool_count", len(tools_used))
